...
function AllMembers(props) {
  ...
  useEffect(() => {
    async function getAllMembersSubscriptionData() {
      ...
      if (memberId != null) {
         let results = await subscriptionsUtils.getAllMembersSubscriptionDataFilterByMemberId(memberId, stateAccessToken);
        if (results.isSuccess) {
         ...
         let results = await subscriptionsUtils.getAllMembersSubscriptionData(stateAccessToken, stateAccessToken);
        if (results.isSuccess) {
          membersSubscriptionData = results.membersDataArr;
        }
        else {
    ...
    <div id="divMembers">
      {
        membersSubscriptionData?.map((memberSubscriptionData, key) => {
          return <div id={memberSubscriptionData.memberData._id}>
