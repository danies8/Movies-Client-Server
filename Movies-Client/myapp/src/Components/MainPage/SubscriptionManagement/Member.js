...
function Member(props) {
    ...
    const { isUserAdmin, hasPermissionForEditSubscriptions, hasPermissionForDeleteSubscriptions,
    ...
    const onEditMember = () => {
        ...
        history.push(`${common.editMemberWithId}/${props.memberSubscriptionData.memberData._id}`)
        props.onUserClickOnEditMemberCallback(props.memberSubscriptionData.memberData._id);
    }
    const onDeleteMember = async () => {
        let results = await subscriptionUtils.deleteMember(props.memberSubscriptionData.memberData._id, stateAccessToken);
        if (results.isSuccess) {
            props.onUserClickOnDeleteMemberCallback(props.memberSubscriptionData.memberData._id);
        }
        else {
    ...
    const toggleSubscirbeToNewMovie = async () => {
        ...
        if (toggleAddMovieArea) {
            ...
            let results = await movieUtils.getAllMoviesNames(stateAccessToken);
            if (results.isSuccess) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async values => {
            ...
            let results = await subscriptionUtils.subscirbeToNewMovie(newMovieDataObj, 
                props.memberSubscriptionData.memberData._id, stateAccessToken);
            if (results.isSuccess) {
                goToAllMembers();
            }
            else {
        ...
        <form onSubmit={formik.handleSubmit} >
            <div className="memberBorderStyle">
                <div className={visibleStyle}>
                    <h6>{props.memberSubscriptionData.memberData.name}</h6>
                    <strong>Email:</strong>{props.memberSubscriptionData.memberData.email} <br />
                    <strong>City:</strong>{props.memberSubscriptionData.memberData.city} <br />
                    {
