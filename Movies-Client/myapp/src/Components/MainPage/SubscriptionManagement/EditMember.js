...
function EditMember(props) {
    ...
    useEffect(() => {
        async function getMemberById() {
            ...
            let results = await subscriptionUtils.getMemberById(memberId, stateAccessToken);
            if (results.isSuccess) {
                setMember(results.member);
                formik.setFieldValue("name", results.member.name, false);
                formik.setFieldValue("email", results.member.email, false);
                formik.setFieldValue("city", results.member.city, false);
            }
            else {
    ...
    const formik = useFormik({
        ...
        onSubmit: async values => {
            ...
            let results = await subscriptionUtils.updateMember(editMember, stateAccessToken);
            if (results.isSuccess) {
                navigateToAllMembers();
            }
            else {
