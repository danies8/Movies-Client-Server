...
function EditMovie(props) {
    ...
    useEffect(() => {
        async function getMovieById() {
            ...
            let results = await movieUtils.getMovieById(movieId, stateAccessToken);
            if (results.isSuccess) {
                setMovie(results.movie);
                formik.setFieldValue("name", results.movie.name, false);
                formik.setFieldValue("genres", results.movie.genres, false);
                formik.setFieldValue("imageUrl", results.movie.image, false);
                formik.setFieldValue("premiered", moment(results.movie.premiered).format('YYYY-MM-DD'), false);
            }
            else {
    ...
    const formik = useFormik({
        ...
        onSubmit: async values => {
            ...
            editMovie.image = values.imageUrl;
            editMovie.premiered = values.premiered;
        
            let results = await movieUtils.updateMovie(editMovie, stateAccessToken);
            if (results.isSuccess) {
                navigateToAllMovies();
            }
            else {
