...
function Movie(props) {
    ...
    const { isUserAdmin, hasPermissionForEditMovies, hasPermissionForDeleteMovies,
    ...
    const onEditMovie = (e) => {
        ...
        setIsVisibleStyle(false);
         history.push(`${common.editMovieWithId}/${props.movieDataObj.movieData._id}`);
        props.onUserClickOnEditMovieCallback(props.movieDataObj.movieData._id);
    }
    const onDeleteMovie = async () => {
        let results = await movieUtils.deleteMovie(props.movieDataObj.movieData._id, stateAccessToken);
        if (results.isSuccess) {
            props.onUserClickOnDeleteMovieCallback(props.movieDataObj.movieData._id);
        }
        else {
            ...
            <form onSubmit={e => onEditMovie(e)}>
                <div className={`${visibleStyle}`}>
                    <Card style={{ width: '16rem' }}>
                        <Card.Img variant="top" src={props.movieDataObj.movieData.image} alt={props.movieDataObj.movieData.name} height="200" width="200" />
                        <Card.Body>
                            <Card.Title>{props.movieDataObj.movieData.name} {new Array(props.movieDataObj.movieData.premiered)[0].split("-")[0]}</Card.Title>
                            <Card.Text >

                                Genres:{new Array(props.movieDataObj.movieData.genres).join(",")}
                            ...
                            <AccordionDetails>
                                <Box >
                                    <ListGroup>{props.movieDataObj.subscribedDataArray?.map((subscribedData, key) => {
                                        return <div key={key}>
                                            <ListGroup.Item key={key} ><NavLink key={key} to={`${common.allMembers}/${subscribedData.memberData._id}`} >{subscribedData.memberData.name} </NavLink>{subscribedData.date} </ListGroup.Item>
                                            <br />
                                        </div>
                                    })}</ListGroup>
                                </Box>
