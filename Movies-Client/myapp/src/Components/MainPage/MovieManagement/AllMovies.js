...
function AllMovies(props) {
  ...
  const { filterMovieName, accessToken } = useContext(AppContext);
   ...
   useEffect(() => {
    async function getAllMoviesData() {
      ...
      if (movieName != undefined && movieName != "") {
        let results = await moviesUtils.getAllMoviesFilterByMovieName(movieName, true, stateAccessToken);
      ...
      // Case of get all movies
        ...
        let results = await moviesUtils.getAllMovies(stateAccessToken);
        if (results.isSuccess) {
      ...
      // Case of filter all movies
        ...
        let results = await moviesUtils.getAllMoviesFilterByMovieName(stateFilterMovieName, false, stateAccessToken);
        if (results.isSuccess) {
          setFilterMovieName("");
          moviesDataArr = results.moviesDataArr;
        }
  ...
  return (
    <div >
      ...
      { moviesDataArr?.map((movieDataObj, key) => {
       return <Grid  id={movieDataObj.movieData._id} item key={key} xs={4}>
